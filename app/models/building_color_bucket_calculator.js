...
define(['underscore', 'd3'], function (_, d3) {
  ...
  BuildingColorBucketCalculator.prototype.cartoCSS = function () {
    ...
    var defaultPolygonLineStyle = 'line-color: #FFF';
    if (this.thresholds) {
      css = this.memoized.cartoCSS[this.fieldName] = _.map(stops, function (stop, i) {
        var min = _.min(gradient.invertExtent(stop));
        var cssText;
        if (i === 0) {
          cssText = cssFillType === 'polygon-fill' ? "[".concat(fieldName, "<").concat(min, "]{").concat(cssFillType, ":").concat(stop, "; ").concat(defaultPolygonLineStyle, "}") : "[".concat(fieldName, "<").concat(min, "]{").concat(cssFillType, ":").concat(stop, "}");
          return cssText;
        }
        cssText = cssFillType === 'polygon-fill' ? "[".concat(fieldName, ">=").concat(min, "]{").concat(cssFillType, ":").concat(stop, "; ").concat(defaultPolygonLineStyle, "}") : "[".concat(fieldName, ">=").concat(min, "]{").concat(cssFillType, ":").concat(stop, "}");
        return cssText;
